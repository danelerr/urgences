...
window.getCurrentLocation = function() {
    ...
    navigator.geolocation.getCurrentPosition(
        position => {
            ...
            // Update location display in modal if open
            ...
            // Success message
            if (locationInfo) {
                locationInfo.innerHTML = `✅ Ubicación obtenida: ${lat.toFixed(4)}, ${lng.toFixed(4)}`;
                locationInfo.style.background = 'linear-gradient(135deg, #d4edda, #c3e6cb)';
        ...
        error => {
            ...
            if (locationInfo) {
                ...
                locationInfo.style.background = 'linear-gradient(135deg, #f8d7da, #f5c6cb)';
...
window.updateLocationInfo = function(lat, lng) {
    const locationInfo = document.getElementById('locationInfo');
    if (locationInfo) {
        locationInfo.innerHTML = `📍 Ubicación seleccionada<br>Lat: ${lat.toFixed(6)}, Lng: ${lng.toFixed(6)}`;
        locationInfo.style.display = 'block';
        locationInfo.style.background = 'linear-gradient(135deg, #e8f8f5, #d5f4e6)';
        locationInfo.style.color = '#2c5530';
    }
}
